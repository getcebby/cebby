...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
    ...
    try {
        ...
        const { data: fbPages } = await supabase.from('facebook_pages').select('*').eq('is_active', true);
...
async function getUrlsFromFbPages(fbPages: FbPage[]) {
    return Promise.all(
        fbPages.map(async (page) => {
            ...
            console.log('🚀 ~ jsonData page:', page.username, jsonData?.edges);

            const urls = jsonData?.edges?.map((item: { node: { url: string } }) => item.node.url);
            return urls;
        }),
    );
}
