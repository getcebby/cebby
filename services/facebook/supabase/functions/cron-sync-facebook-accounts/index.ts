    ...
    try {
        ...
        const fetchPromises = accounts.map((account: Tables<'accounts'>) => {
            ...
            return fetch(`${supabaseUrl}/functions/v1/retrieve-and-sync-to-db-events`, {
                ...
                .catch((error) => {
                    console.error(`Error calling function for account ${account.id}:`, error);
                });
        });
        ...
        const data = {
            message: `Successfully called functions to sync events to db with ${accounts.length} accounts`,
        };
