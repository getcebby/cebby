...
Deno.serve(async (req) => {
  try {
    ...
    console.log(
      `Retrieved ${events.length} events from Facebook for account: ${account_id}`,
    );

    const mappedEvents = mapEventsToDB(events, account);
    console.log(
      `Mapped ${mappedEvents.length} events for account: ${account_id}`,
    );

    const savedResult = await saveEventsToDB(mappedEvents);
    console.log(
      `Saved events to DB for account: ${account_id}. Result:`,
      savedResult,
    );
    ...
    return new Response(
      JSON.stringify({
        message:
          `Successfully processed ${events.length} events for account: ${account_id}`,
